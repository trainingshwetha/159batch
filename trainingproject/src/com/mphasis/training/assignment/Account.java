...
public  class Account {
	...
	public double withdraw(int amount)throws InsufficientBalance {
		if(this.balance > (amount+1000) )
		{
			...
			throw new InsufficientBalance("minimum balance 1000");
		}
		else {
			throw new InsufficientBalance("insuffienct balance");
			
		}
		return balance;
		
	}
	public double deposit(int amount)throws InsufficientBalance {
		if((amount%100)==0) {
		balance=balance+amount;
		}else {
			throw new InsufficientBalance("only multiples of 100 can deposit");
		}
		return balance;
